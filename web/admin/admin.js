...
function exportDev() {
  collectCurrent();
  download(
    'enemy-profile-dev-backlog.json',
    JSON.stringify(
      { createdAt: new Date().toISOString(), items: devBacklog },
      null,
      2,
    ) + '\n',
  );
...
function renderLists() {
  ...
  for (const rank of rankOrder) {
    ...
    head.innerHTML = `<span>${rankLabels[rank]}</span><span>${profiles.length}</span>`;
    ...
    for (const { profile, index } of profiles) {
      ...
      button.textContent = profile.name || profile.key;
...
function outcomeRow(outcome) {
  ...
  row.innerHTML = [
    ...
    input('Label', 'out-label', outcome.label || ''),
...
function addDevNote() {
  ...
  devBacklog.push({ enemyKey: selectedProfile.key, enemyName: selectedProfile.name, rank: selectedProfile.rank, text, createdAt: new Date().toISOString() });
  $('devNoteInput').value = '';
  renderDevNotes();
}
...
function renderDevNotes() {
  $('devNotes').innerHTML = devBacklog.filter((item) => !selectedProfile || item.enemyKey === selectedProfile.key).map((item) => `<div class="dev-note"><strong>${item.enemyName}</strong><br>${escapeText(item.text)}</div>`).join('');
}
function bindBasics() {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('exportDevBtn').onclick = exportDev;
